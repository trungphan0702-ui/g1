...
def compute_thd(signal: np.ndarray, fs: int, freq: float, max_h: int = 5) -> Dict[str, Any]:
    ...
    band_bins = 2  # capture leakage around the fundamental
    ...
    power_total = float(np.sum(mag ** 2))
    power_fund = float(fund ** 2)
    noise_power = max(power_total - power_fund, 0.0)
    thdn_ratio = np.sqrt(noise_power / power_fund) if power_fund > 0 else 0.0
    ...
    if os.getenv("DSP_DEBUG"):
        print(f"[DSP_DEBUG] compute_thd: fund_idx={fund_idx}, fund_mag={fund:.3e}, thd_db={normalized['thd_db']:.2f}, thdn_db={normalized['thdn_db']:.2f}")
