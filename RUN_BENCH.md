...
- **Compare**: đặt `input_wav` và `output_wav` cho từng case trong `compare_cases`; dùng `compare_max_lag_ms` để giới hạn cửa sổ tìm độ trễ (mặc định 200 ms). Có thể dùng `type: synthetic` để tạo cặp tín hiệu giả lập (delay/gain/noise/harmonic) phục vụ sanity check, và tinh chỉnh `compare_trim_start/end` cùng `compare_verdict_thresholds` (ngưỡng SNR/gain/FR/THD/clipping) cho nhãn GOOD/OK/BAD.
...
- compare_synthetic_example [compare] | Latency 4.00 ms, Gain -6.00 dB, SNR 45.0 dB, Verdict OK
